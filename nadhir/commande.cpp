...
void  Commande::telechargerPDF(){
                     ...
                     QPdfWriter pdf("C:\\Users\\pc\\OneDrive\\Documents\\nadhir\\import.pdf");
                     QPainter painter(&pdf);
                    int i = 4000;
                         painter.setPen(Qt::blue);
                         painter.setFont(QFont("Arial", 30));
                         painter.drawText(1100,1200,"GESTION commandes");
                         painter.setPen(Qt::black);
                         painter.setFont(QFont("Arial",14));
                         painter.drawRect(100,500,7300,2600);
                         painter.drawRect(0,3000,9600,500);
                         painter.setFont(QFont("Arial",8));
                         painter.drawText(300,3300,"NUMERO_DE_COMMANDE");
                         painter.drawText(1300,3300,"DATE_DE_LIVRASION");
                         painter.drawText(2300,3300,"REFERENCE");
                         painter.drawText(3300,3300,"TOTAL_DE_COMMANDE");
                         painter.drawText(4300,3300,"NATIONALITE");



                         QSqlQuery query;

                         query.prepare("select NUMERO_DE_COMMANDE,DATE_DE_LIVRASION,REFERENCE,TOTAL_DE_COMMANDE,NATIONALITE from COMMANDE");
                         query.exec();
                         while (query.next())
                         {

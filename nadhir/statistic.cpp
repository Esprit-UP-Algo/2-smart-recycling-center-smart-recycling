...
QSqlQuery qry;
int i=0;
qry.exec("SELECT NUMERO_DE_COMMANDE FROM COMMANDE");
while (qry.next())
{
    QString NUMERO_DE_COMMANDE = qry.value(0).toString();
    i++;
    *ticks<<i;
    *labels <<NUMERO_DE_COMMANDE;
}
}
...
QLinearGradient gradient(0, 0, 0, 400);
                  ...
                  amande->setStackingGap(1);
                   //couleurs
                  amande->setName("Repartition des commande selon nationalite ");
                  ...
                  QSqlQuery q1("SELECT TOTAL_DE_COMMANDE FROM COMMANDE");
                  while (q1.next()) {

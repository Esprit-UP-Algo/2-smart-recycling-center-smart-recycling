...
void Smtp::sslErrors(const QList<QSslError> &errors)
{
    ...
    // Gérez les erreurs SSL selon vos besoins, par exemple, en fermant la connexion.
    socket->abort();
    state = Close;
}
...
void Smtp::encrypted()
{
    ...
    // Continue avec le processus d'envoi de courriel ici, par exemple, émettre un signal pour informer que la connexion est prête.
    emit status( tr( "Encrypted connection established" ) );
}
